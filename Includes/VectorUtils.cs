...
namespace IngameScript
{
	partial class Program
	{
        public static class VectorUtils
        {
            ...
            {
                double lenSq = two.LengthSquared();
                if (lenSq < 1e-8)
                    return Vector3D.Zero; // or throw/log depending on intent

                return Vector3D.Dot(one, two) / lenSq * two;
            }
            ...
            public static Vector3D ProjectOnPlane(Vector3D planeLeft, Vector3D planeForward, Vector3D direction)
            {
                ...
                if (normal.LengthSquared() < 1e-8) // handle degenerate plane
                    return Vector3D.Zero; // or direction, or throw/log depending on your use case

                normal.Normalize();

                return direction - Project(direction, normal);
            }
            ...
            {
                if (planeNormal.LengthSquared() < 1e-8) // handle degenerate plane
                    return Vector3D.Zero; // or direction, or throw/log depending on your use case
            ...
            public static double SignedAngle(Vector3D from, Vector3D to, Vector3D axis)
            {
                ...
                double input = Vector3D.Dot(axis, Vector3D.Cross(from, to));
